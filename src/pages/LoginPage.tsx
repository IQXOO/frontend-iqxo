...
export default function LoginPage() {
  ...
  const handleAppleSignIn = async () => {
    ...
    setError(null);
    try {
      devLog("Auth", "Apple OAuth initiated");
      const { error: oauthError } = await supabase.auth.signInWithOAuth({
        provider: "apple",
        options: {
          redirectTo: buildOAuthRedirectUrl(),
      ...
      devError("Auth", "Apple OAuth failed", err);
      const message = getFriendlyErrorMessage(err, "Apple sign-in failed");
      setError(message);
      toast({
        title: "Couldn't sign in with Apple",
        description: message,
        ...
        <div className="login-card">
          ...
          {!forgotOpen ? (
            <>
              ...
              {/* Social Login */}
              <button className="social-btn" onClick={handleGoogleSignIn} disabled={googleLoading || appleLoading || loading}>
                {googleLoading ? (
              ...
              <button className="social-btn" onClick={handleAppleSignIn} disabled={googleLoading || appleLoading || loading}>
                {appleLoading ? (
                  <div className="w-5 h-5 border-2 border-cyan/30 border-t-cyan rounded-full animate-spin" />
                ) : (
                  <svg viewBox="0 0 24 24" fill="currentColor"><path d="M12.152 6.896c-.948 0-2.415-1.078-3.96-1.04-2.04.027-3.91 1.183-4.961 3.014-2.117 3.675-.54 9.103 1.51 12.06 1.005 1.45 2.187 3.068 3.76 3.007 1.516-.06 2.09-.974 3.916-.974 1.815 0 2.342.974 3.931.942 1.62-.027 2.656-1.464 3.648-2.906 1.15-1.68 1.625-3.308 1.653-3.393-.036-.015-3.176-1.218-3.21-4.821-.027-3.007 2.477-4.453 2.593-4.526-1.41-2.069-3.593-2.3-4.364-2.355-2.02-.166-3.91 1.233-4.787 1.233zM14.92 3.82c.846-1.024 1.41-2.454 1.255-3.82-1.173.047-2.59.782-3.433 1.766-.739.849-1.385 2.296-1.211 3.639 1.305.101 2.63-.642 3.389-1.585z"/></svg>
                )}
                <span>{t.apple}</span>
              </button>
